...
$sql="select count(*) from olddeal_user RIGHT JoIN pub ON  olddeal_user.uname = pub.uname where (pone LIKE '%$keyWord%' OR pname LIKE '%$keyWord%') AND STAUS=1";
...
$sql="select *from olddeal_user RIGHT JoIN pub ON  olddeal_user.uname = pub.uname where (pone LIKE '%$keyWord%' OR pname LIKE '%$keyWord%') AND STAUS=1 limit $start,$count";;
$result=mysqli_query($conn,$sql);
