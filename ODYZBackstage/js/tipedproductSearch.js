$("#product .form1").on('click','#search-btn',function(){
    ...
    $('#product .pager').on('click','a',function(event){
        ...
        searchLoadTip(pageNum,kw)
    });
});
//功能点五 用户点击分页条号时实现异步加载
/**功能点5：用户点击分页条中的页号时，实现数据的异步加载**/

function searchLoadTip(pageNum,kw){
    $.ajax({
        url:"data/tipedproductSearch.php?pageNum="+pageNum+"&kw="+kw,
        data: {mname: sessionStorage['loginName'],uname:sessionStorage['tipeduser']},
